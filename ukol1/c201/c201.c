...
void List_Dispose( List *list ) {
	...
	while(list->firstElement != NULL) //iterace dokud neni seznam list prazdny
	{
		temp = list->firstElement->nextElement; //ulozeni nasledujiciho elementu pred odstranenim
		free(list->firstElement);
		list->firstElement = temp;//posunuti na next element
	}

	list->activeElement=NULL; //odstraneni aktvity
...
void List_InsertFirst( List *list, int data ) {
	...
	if(new != NULL) //dostatek mista
	{
		new->data = data;
		new->nextElement = list->firstElement;// navazani noveho elementu na predchazejici
		list->firstElement = new;//posunuti uk first na 1.prvek
	}
	else List_Error();	//pri nedostatku mista 
}
...
void List_GetFirst( List *list, int *dataPtr ) {
	if(list->firstElement != NULL) 
		*dataPtr = list->firstElement->data;//pokud seznam neni prazdny predat data pomoci uk
	else 
		List_Error();
}
...
void List_DeleteFirst( List *list ) {
	if(list->firstElement!=NULL)//neprazdny seznam
	{
		if(list->activeElement == list->firstElement) 
			list->activeElement = NULL; //ztrata aktivity
		
		ListElementPtr temp = list->firstElement->nextElement;//odkaz na 2.prvek seznamu
		free(list->firstElement);
		list->firstElement = temp;//posunuti uk first na nasledujici prvek
	}
	else return;
}
...
void List_DeleteAfter( List *list ) {
	...
	{//      aktivni list      &&      aktivni prvek neni poslednim prvkem                     
		
		ListElementPtr temp = list->activeElement->nextElement;//ulozeni dalsiho prvku za aktivnim
		list->activeElement->nextElement = temp->nextElement;  //navazani aktivniho prvku na dalsi platny prvek
		free((void*)temp);							   		   //|---| |---| |---|
	}														   //|AKT| |DEL| |NEX|
	else return;											   //|---| |---| |---|
}															   // ||           ||
...
void List_InsertAfter( List *list, int data ) {
	if(list->activeElement != NULL)//aktitvni seznam
	{
		ListElementPtr new = malloc(sizeof(struct ListElement));
		if(new != NULL)
		{
			new->data = data;
			new->nextElement = list->activeElement->nextElement;//navazani na nasledujici prvek
			list->activeElement->nextElement = new;//vlozeni za akt. prvek
		}
		else List_Error();
	}
}
...
void List_Next( List *list ) {
	if(list->activeElement !=NULL)
	{
		list->activeElement= list->activeElement->nextElement;//posunuti
	}
}
...
int List_IsActive( List *list ) {
	return (list->activeElement!=NULL);//aktivni uk neni NULL => aktivni seznam
}
