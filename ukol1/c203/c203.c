...
void Queue_Init( Queue *queue ) {
	if(queue != NULL)
	{
		for(int i = 0 ; i < QUEUE_SIZE ; ++i) //vyplneni pole *
			queue->array[i]='*';

		queue->firstIndex = 0;
		queue->freeIndex = 0;
	}
	else Queue_Error(QERR_INIT);
...
int nextIndex( int index ) {
	return (index + 1) % QUEUE_SIZE; //vraceni dalsiho idx nebo 0 pri preteceni
}
...
int Queue_IsEmpty( const Queue *queue ) {
	return (queue->freeIndex == 0);//uk freeIndex se rovna 0 => nebyla vlozena zadna hodnota
}
...
int Queue_IsFull( const Queue *queue ) {
	...
	return (nextIndex(queue->freeIndex) == queue->firstIndex);//pri preteceni se bude free+1 a first rovnat
}															  // => fornta je plna
...
void Queue_Remove( Queue *queue ) {
	if(!Queue_IsEmpty(queue))
		queue->firstIndex = nextIndex(queue->firstIndex);//posunuti uk firstIndex za ucelem odstraneni 1.znaku
...
void Queue_Dequeue( Queue *queue, char *dataPtr ) {
	if(!Queue_IsEmpty(queue))//neprazdna fronta
	{
		Queue_Front(queue,dataPtr);//vraceni 1.znaku
		Queue_Remove(queue);	   //odstraneni tehoz znaku 
	}
...
void Queue_Enqueue( Queue *queue, char data ) {
	...
	{
		queue->array[queue->freeIndex] = data;		   //vlozeni znaku
		queue->freeIndex = nextIndex(queue->freeIndex);//aktualizace uk freeIndex
	}
